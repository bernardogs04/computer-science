...
void formatVeiculo(Veiculo v, char* buffer){
	...
	sprintf(buffer, "[%d ## %s ## %s ## %d ## %s ## %s ## %d ## %s ## %s ## %s ## %s ## %s ## %s ## %s ## %s]",
			v.id, v.marca, v.modelo, v.ano, v.categoria, combustivelStr, v.cilindros,
			cilindradaStr, v.transmissao, v.tracao, consumoCidadeStr, consumoEstradaStr,
			co2Str, turboStr, dataRegistroStr);
}
...
int main(){
	...
	while (scanf("%d", &IdProcurar) == 1 && IdProcurar != -1) {
		for (int i = 0; i < n; i++) {
			if (veiculos[i].id == IdProcurar) {
